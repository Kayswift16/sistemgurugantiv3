  ...
  return (
    <Dialog open={isOpen} onClose={onClose} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="w-full max-w-5xl rounded-2xl bg-white p-6 shadow-xl overflow-y-auto max-h-[90vh]">
          <Dialog.Title className="text-xl font-bold text-slate-800 mb-4">
            📅 Jadual Waktu Semasa
          </Dialog.Title>

          <p className="text-sm text-slate-600 mb-6">
            Ini ialah jadual waktu yang sedang digunakan oleh sistem. 
                  ...
                  <tbody>
                    {groupedByDay[day]?.map((entry, i) => (
                      <tr key={i} className="border-b border-slate-200">
                        ...
                        <td className="px-3 py-2">{entry.teacherId}</td>
                      </tr>
                    )) || (
                      <tr>
