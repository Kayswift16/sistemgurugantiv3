...
const getTimeSlotsForDay = (day: string): string[] => {
  ...
  if (upperDay === "KHAMIS" || upperDay === "JUMAAT") {
    return allTimeSlots.slice(0, 10); // until 12:10
  }
  return allTimeSlots; // until 12:40
};

// Raw timetable (from Jadual Waktu Induk 2025)
const rawData: Record<string, Record<string, string[]>> = {
  ...
  RABU: {
    ...
    "TAHUN 4": ["PJ/NZ", "PJ/NZ", "BI/MN", "SEJ/AY", "SEJ/AY", "PM/SD PI/IM", "PM/SD PI/IM", "BM/NS", "BM/NS", "MZ/KZ", ""],
  ...
  KHAMIS: {
    ...
    "TAHUN 2": ["BIFS", "BI/FS", "PJ/IM", "PJ/IM", "PK/AY", "PS/FS", "PS/FS", "BM/JL", "BM/JL", ""],
    "TAHUN 3": ["BM/JL", "BM/JL", "MZ/SD", "SN/KV", "SN/KV", "BI/BB", "BI/BB", "MT/NS", "MT/NS", ""],
...
Object.entries(rawData).forEach(([day, classes]) => {
  ...
  Object.entries(classes).forEach(([className, subjects]) => {
    subjects.forEach((subjectTeacher, index) => {
      ...
      if (!time || !subjectTeacher || ["PH", "H", "R"].includes(subjectTeacher)) return;
      ...
      if (subjectTeacher.includes(' ')) {
        ...
        TIMETABLE.push({ day, time, class: className, subjects: parsedSubjects, isJoint: true });
      } else if (subjectTeacher.includes('/')) {
        const [subject, teacherId] = subjectTeacher.split('/');
        TIMETABLE.push({ day, time, class: className, subjects: [{ subject, teacherId }] });
      }
    });
  });
});
